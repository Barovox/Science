...
const TrongNuoc = ({ view }: { view?: string }) => {
  ...
  const handleInputChange = (index: number, key: string, value: string) => {
    ...
    if (
      (updatedRows[index].tenGiai ||
        updatedRows[index].capKhenThuong ||
        updatedRows[index].soQuyetDinh) &&
      index === rows.length - 1
    ) {
      updatedRows.push({
        tenGiai: "",
    ...
    const filteredRows = updatedRows.filter(
      (row: any, i: any) =>
        ...
        row.tenGiai.trim() !== "" ||
        row.capKhenThuong.trim() !== "" ||
        row.soQuyetDinh.trim() !== ""
    );
    ...
    try {
      ...
      const res = await axios.put(
        `${SERVER_API_URL}/LyLich2/Update/info/${user.IDUser}`,
        {
          ...
          giaiThuong_TrongNuoc: JSON.stringify(nonEmptyRows),
        },
        {
          headers: {
            Authorization: `Bearer ${user.Token}`,
    ...
    <>
      ...
      <Table.ScrollContainer minWidth={500} type="native">
        <Table striped highlightOnHover withTableBorder>
          <Table.Thead>
            <Table.Tr>
              <Table.Th>Tên khen thưởng</Table.Th>
              <Table.Th>Cấp khen thưởng</Table.Th>
              <Table.Th>Số quyết định</Table.Th>
            </Table.Tr>
          </Table.Thead>
          <Table.Tbody>
            {rows.map((row, index) => (
              <Table.Tr key={index}>
                <Table.Td>
                  <Textarea
                    autosize
                    variant="unstyled"
                    placeholder={!viewer ? "Tên giải..." : ""}
                    value={row.tenGiai}
                    onChange={(e) =>
                      handleInputChange(index, "tenGiai", e.target.value)
                    }
                    readOnly={viewer}
                  />
                </Table.Td>
                <Table.Td>
                  <Textarea
                    autosize
                    variant="unstyled"
                    placeholder={!viewer ? "Cấp khen thưởng..." : ""}
                    value={row.capKhenThuong}
                    onChange={(e) =>
                      handleInputChange(index, "capKhenThuong", e.target.value)
                    }
                    readOnly={viewer}
                  />
                </Table.Td>
                <Table.Td>
                  <Textarea
                    autosize
                    variant="unstyled"
                    placeholder={!viewer ? "Số quyết định..." : ""}
                    value={row.soQuyetDinh}
                    onChange={(e) =>
                      handleInputChange(index, "soQuyetDinh", e.target.value)
                    }
                    readOnly={viewer}
                  />
                </Table.Td>
