...
const TaoTaiKhoan = ({
  ...
  const handleCreateAccount = async (e: Event) => {
    ...
    const id = notifications.show({
      loading: true,
      title: "Hành động",
      message: "Đang tạo tài khoản phản biện...",
      autoClose: false,
    ...
    try {
      ...
      if (res.status === 200) {
        notifications.update({
          ...
          color: "teal",
          title: "Hành động",
          message: `Đã tạo tài khoản phản biện thành công.${
            sendMailAndInvite
              ? ` Đang gửi lời mời phản biện đến email "${createForm.email}".`
              : ""
          }`,
    ...
    try {
      ...
      if (res.status === 200) {
        notifications.show({
          color: "teal",
          title: "Hành động",
          message: `Đã thêm phản biện "${createForm.hoTen}" vào bài báo.`,
          autoClose: 5000,
        });
      }
    } catch (error) {
    ...
    try {
      const res = await axios.post(
        `${MAILER_API_URL}/send-email/${
          message ? "gui-phan-bien-kem-tin-nhan" : "gui-phan-bien"
        }`,
        {
          to: createForm.email,
          mailData,
          ...
          <Grid.Col span={6}>
            <Fieldset legend="Tùy chọn">
              <Checkbox
                ...
                label="Gửi lời mời phản biện qua email tài khoản đăng ký sau khi tạo."
                disabled={isWaiting}
              />
            ...
            <Button mt="md" fullWidth type="submit" loading={isWaiting}>
              {!sendMailAndInvite
                ...
                : sendMailAndInvite &&
                  `Tạo và gửi lời mời${
                    isMultiBB ? " hàng loạt" : ""
                  } đến phản biện${message ? " (kèm lời nhắn)" : ""}`}
            </Button>
          </Grid.Col>
        </Grid>
      </form>
