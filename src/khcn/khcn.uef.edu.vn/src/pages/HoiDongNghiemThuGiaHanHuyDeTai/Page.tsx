...
const HoiDongNghiemThuDeTaiCBGVNVPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(
        `${SERVER_API_URL}/HDNghiemThuDT_CBGVNV/Get-all?startTimes=${timeline}`,
        {
          headers: {
  ...
  const hanldeOpenModalInviteMultiBB = () => {
    modals.open({
      title: "Mời nghiệm thu cho các đề tài được chọn",
      size: "100%",
  ...
  return (
    <>
      <Title order={2} mb="sm">
        Điều phối công tác nghiệm thu / gia hạn / hủy đề tài
      </Title>
                        ...
                        <Text>
                          <Badge
                            color={getStatusNghiemThuColor(
                              dmxdSV.status_nghiemThu || "Đang thực hiện",
                            )}
                          >
                            {dmxdSV.status_nghiemThu || "Đang thực hiện"}
                          </Badge>
...
export default HoiDongNghiemThuDeTaiCBGVNVPage;
