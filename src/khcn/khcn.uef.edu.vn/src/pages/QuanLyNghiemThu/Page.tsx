...
const HoiDongXetDuyetDeTaiCBGVNVPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(
        `${SERVER_API_URL}/DanhMucXetDuyetSinhVien/Get-all-nghiem-thu?startTimes=${timeline}&quyen=${user.Role}`,
        {
          headers: {
  ...
  return (
    <>
      <Title order={2} mb="sm">
        Quản lý kết quả nghiệm thu, thanh lý đề tài
      </Title>
      ...
      <Group justify="space-between" mt="md">
        <div>
          <TextInput
            label={
              <>
                <b>{filteredBaiBao.length}</b> đề tài
              </>
            }
              ...
              currentItems.map((dmxdSV: any) => {
                return (
                  ...
                  >
                    ...
                    <Table.Td ta="center">
                      <Text>
                        {dmxdSV.danhSachHDNghiemThu
                          ? JSON.parse(dmxdSV.danhSachHDNghiemThu).filter(
                      ...
                      <Text>
                        {dmxdSV.hoiDongDanhGia
                          ? JSON.parse(dmxdSV.hoiDongDanhGia).filter(
                              (dg: any) =>
                                dg.fromHoiDongNghiemThu && dg.ketLuan == "Đạt",
                            ).length
                          : 0}
                      </Text>
                    </Table.Td>
                    <Table.Td ta="center">
                      <Text>
                        {dmxdSV.hoiDongDanhGia
                          ? JSON.parse(dmxdSV.hoiDongDanhGia).filter(
                              (dg: any) =>
                                dg.fromHoiDongNghiemThu &&
                                dg.ketLuan == "Không đạt",
                            ).length
                          : 0}
                      </Text>
                    </Table.Td>
                    <Table.Td>
                      <Text>
                        <Tooltip.Floating
                          label={dmxdSV.status_nghiemThu}
                          position="left"
                        >
                          <Badge
                            color={getStatusNghiemThuColor(
                              dmxdSV.status_nghiemThu,
                            )}
                          >
                            {dmxdSV.status_nghiemThu}
                          </Badge>
